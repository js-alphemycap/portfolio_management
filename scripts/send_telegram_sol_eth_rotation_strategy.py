#!/usr/bin/env python3
"""Send the SOL/ETH rotation signal message to a Telegram chat."""
...
from portfolio_management.strategies.sol_eth_rotation_strategy_telegram import (
    build_sol_eth_rotation_telegram_message,
)

FULL_HISTORY_START_DATE = datetime(2021, 12, 1, tzinfo=timezone.utc)


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Send the SOL/ETH rotation daily signal message to Telegram."
    )
    parser.add_argument(
        "chat_id",
        nargs="?",
...
def main() -> None:
    ...
    snapshot = generate_sol_eth_rotation_snapshot(
        sol_close=sol_close,
        eth_close=eth_close,
        config=strategy_conf,
    )
    message = build_sol_eth_rotation_telegram_message(
        snapshot=snapshot,
        config=strategy_conf,
    )
    ...
    emit_telegram_message(
        ...
        message_label="📨 Telegram SOL/ETH rotation strategy message:",
